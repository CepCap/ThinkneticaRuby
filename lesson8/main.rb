...
def create_route
  ...
  start = gets.chomp
  start_station = @stations.find { |station| station.name.casecmp(start).zero? }
  puts 'Выберите конечную станцию: '
  finish = gets.chomp
  finish_station = @stations.find { |station| station.name.casecmp(finish).zero? }
  @routes << Route.new(start_station, finish_station)
end

def route_assign
  puts 'Выберите номер поезда: '
  @trains.each_with_index { |train, index| puts "#{index}. => #{train.model}" }
...
def stations_show
  @stations.each do |station|
    ...
    puts station.trains.each(&:model)
  end
end

def wagon_use
  puts 'Выберите номер поезда:'
  @trains.each_with_index { |train, index| print "#{index}. => #{train.model}" }
  chosen_train = gets.to_i
  train = @trains[chosen_train]
  puts 'Выберите номер вагона: '
  @wagons.each_with_index { |wagon, index| print "#{index}. => #{wagon.id}" }
...
def route_change
  ...
  if input == 'добавить'
    ...
    @stations.each_with_index { |station_add, index| puts "#{index}. => #{station_add.name}" }
    ...
    @stations.each_with_index { |station_del, index| puts "#{index}. => #{station_del.name}" }
    chosen_station = gets.to_i
    station = @stations[chosen_station]
...
def change_wagons
  ...
  puts 'Выберите вагон'
  wagons.each_with_index { |wagon, index| puts "#{index}. => #{wagon.id}" }
  chosen_wagon = gets.to_i
  wagon = wagons[chosen_wagon]
